...
export default function StatCard({
  ...
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-white/10 bg-[#00000033] p-6 backdrop-blur-md transition duration-300 hover:border-white/15 hover:shadow-[0_20px_60px_rgba(0,18,184,0.15)]">
      <div
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-3xl font-bold tracking-tight text-tsai-text">{value}</p>
          {change ? (
            <p className={`mt-2 text-sm font-medium ${trendColor[trend]}`}>{change}</p>
          ) : null}
        </div>
